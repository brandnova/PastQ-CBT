...
const TrialMode = ({ user, setUser }) => {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      // Update backend
      const updateResponse = await axios.patch('https://qbank.backend.kumotechs.com/api/user', { trial_calls: updatedTrialCalls }, {
      ...
      if (updatedTrialCalls <= 0) {
        await axios.patch('https://qbank.backend.kumotechs.com/api/user', { trial_complete: true }, {
          headers: { Authorization: `Bearer ${localStorage.getItem('access_token')}` },
          withCredentials: true,
        });
        setUser(prevUser => ({
